...
import { Server } from "http";  // 추가된 부분

const app = express();

app.get("/", (req: Request, res: Response) => res.send("Express on Vercel"));

app.get("/get-directions", async (req: Request, res: Response) => {
...
if (process.env.NODE_ENV !== "production") {
  ...
  const server: Server = app.listen(PORT, () =>  // 타입 명시
    console.log(`Server ready on port ${(server.address() as any).port}.`)
  );
}
